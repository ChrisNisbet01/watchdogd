...
open_and_configure_watchdog(struct watchdog_context_st * const context)
{
    ...
    int const real_wd_timeout_secs = get_watchdog_secs(context->watchdog_fd);
    int const wd_keepalive_secs = context->watchdog_keep_alive_msecs / 1000;

    if (real_wd_timeout_secs < 0)
    {
        perror("Error while getting watchdog timeout");
    }
    else if (real_wd_timeout_secs <= wd_keepalive_secs)
    {
        fprintf(
            stderr,
            "Warning: Watchdog counter (%u secs) <= keepalive interval (%u secs).\n",
            real_wd_timeout_secs,
            wd_keepalive_secs);
        fprintf(stderr, "The watchdog will probably trigger\n");
    }

    debug("watchdog timeout secs %u\n", real_wd_timeout_secs);
    debug("keepalive secs %u\n", wd_keepalive_secs);
